...
const fetchRated = () => {
  return fetch(`https://api.themoviedb.org/3/trending/movie/day?api_key=${apiKey}`).then(res => res.json()).then(({ results }) => results)
}

const fetchByQuery = query => {
  if (query) {
    return fetch(`${baseUrl}search/movie${query}&api_key=${apiKey}`).then(res => res.json()).then(({ results }) => results);
  }
}
...
const fetchById = (movieId, type) => {
  switch (type) {
    ...
    case 'reviews':
      return fetch(`${baseUrl}movie/${movieId}/reviews?api_key=${apiKey}`).then(res => res.json());
    default:
